...
sys.path.insert(-2, '../..')
from config import *
path_input = os.path.join(path_raw_data, r"db_original_gamma.xlsx")
path_output = path_iteration_output_gamma
    ...
    for seed in range(81,91):
        ...
        for sheet_name in ["reference", "sce1", "sce2", "sce3", "sce4", "sce5"]:    
            ...
            for sim in tqdm(range(1, n_sim + 1), desc = f"Cenário: {sheet_name} | Simulações: {n_sim} | Seed: {seed}"):
                ...
                df = activity_duration (df, dist = "gamma") #create real duration column
