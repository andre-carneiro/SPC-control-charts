...
def import_confusion_matrix(path_input_confusion_matrix, scenario):
    """
    ...
    df_cpi = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_cpi_gamma.pkl".format(scenario)))
    df_spi = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_spi_gamma.pkl".format(scenario)))
    df_tpi = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_tpi_gamma.pkl".format(scenario)))
    df_dpi = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_dpi_gamma.pkl".format(scenario)))
    df_min = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_min_t_gamma.pkl".format(scenario)))
    df_max = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_max_t_gamma.pkl".format(scenario)))
    df_pdt = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_pdt_t_gamma.pkl".format(scenario)))
    df_avg = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_avg_t_gamma.pkl".format(scenario)))
    df_med = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_med_t_gamma.pkl".format(scenario)))
    df_amp = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_amp_t_gamma.pkl".format(scenario)))
    df_std = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_std_t_gamma.pkl".format(scenario)))
    df_var = pd.read_pickle(os.path.join(path_input_confusion_matrix, r"df_confusion_matrix_{}_var_t_gamma.pkl".format(scenario)))
        
    return (df_cpi, df_spi, df_tpi, df_dpi, df_min, df_max, df_pdt, df_avg, df_med, df_amp, df_std, df_var)

def slice_df_20 (df):
    
    #slice no df para pegar apenas os valores multiplos de 20
    df = df.loc[range (20,330,20), :].copy()
...
def calculate_project_level_metrics(df):
    ...
    return tmp_project_level[["detection_performance", "probability_overreaction", "efficiency", "reliability"]].copy()
...
def calculate_period_level_metrics(df):
    ...
    return tmp_period_level[["signal_density", "signal_redundancy", "signal_efficiency", "signal_reliability"]]


def concatenate_adjust_df(tmp_project_level, tmp_period_level, scenario, indicator_name):
    """
    ...
    tmp_melt["distribution"] = "gamma"

    #melt df
    tmp_melt = pd.melt(tmp_melt, 
                        id_vars = ["time", "scenario", "indicator", "distribution"], 
...
def export_performance_metrics(df_final, path_output):
    ...
    """
    df_final.to_pickle(os.path.join(path_output, r"df_performance_metrics_gamma.pkl"))