...
def calculate_period_level_metrics(df):
    ...
    tmp_period_level["total_projects_acum"] = tmp_period_level[["TP_acum", "FP_acum", "TN_acum", "FN_acum"]].sum(axis = 1)
    ...
    tmp_period_level["signal_accuracy"] = (tmp_period_level["TP_acum"] + tmp_period_level["TN_acum"])/tmp_period_level["total_projects_acum"]
...
def export_performance_metrics(df_final, path_output, distribution):
    """
    Função que exporta o df de métricas de performance
    """
    df_final.to_pickle(os.path.join(path_output, r"df_performance_metrics_boots_{}.pkl".format(distribution)))